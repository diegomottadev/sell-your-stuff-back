...
async function usuarioNoExiste(usuario, done) {
  try {
    let usuarios = await Usuario.find().or([{ username: usuario.username }, { email: usuario.email }])
...
describe('Usuarios', () => {
  ...
  describe('POST /login', () => {
    ...
    test(
      'Usuario registrado debería obtener un JWT token al hacer login con credenciales correctas',
      (done) => {
        let usuario = {
          username: 'daniel',
          email: 'daniel@gmail.com',
        ...
        new Usuario({
          ...
          request(app)
            ...
            .end((err, res) => {
              expect(res.status).toBe(200)
              expect(res.body.token).toEqual(jwt.sign({ id: nuevoUsuario._id }, config.jwt.secreto, { expiresIn: config.jwt.tiempoDeExpiración }))
              done()
            })
          })
          .catch(err => {
    ...
    test(
      'Al hacer login no debe importar la capitalización del username',
      (done) => {
        let usuario = {
          username: 'daniel',
          email: 'daniel@gmail.com',
